...
// import { JSDOM } from "jsdom"; // para simular o browser
...
//     const btn = makeBtnElement();
//     jest
//       // .spyOn(global.document, document.querySelectorAll.name)
...
describe("#View - test suite for presentation layer ", () => {
  ...
  test("#onStartClicked", async () => {
    ...
    jest.spyOn(view, view.toggleBtnStart.name).mockReturnValue();

    jest.spyOn(view, view.onBtnClick.name).mockResolvedValue();

    jest.spyOn(view, "changeCommandBtnsVisibility").mockReturnValue();

    jest.spyOn(view, view.notIsUnassignedButton.name).mockReturnValue(true);

    jest.spyOn(view, view.setupBtnAction.name).mockReturnValue();
    ...
    jest.spyOn(document, "querySelectorAll").mockReturnValueOnce([btn]);
  ...
  test("#onCommandClick", async () => {
    ...
    jest.spyOn(view, view.toggleDisableCommandBtn.name).mockReturnValue();
    ...
    expect(view.toggleDisableCommandBtn).toHaveBeenNthCalledWith(
      1,
