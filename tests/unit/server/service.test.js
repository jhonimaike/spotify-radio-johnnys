...
// const {
//   dir: { fxDirectory, publicDirectory },
...
describe("#Service - test suite for core processing", () => {
  ...
  test("#getFileInfo", async () => {
    ...
    const expectedResult = {
      type: ".mp3",
      name: `${config.dir.pulicDirectory}/${currentSong}`,
    };
  ...
  test("#getFileStream", async () => {
    const currentReadable = TestUtil.generateReadableStream(["abc"]);
    const currentSong = `mySong.mp3`;
    const currentSongFullPath = `${config.dir.pulicDirectory}/${currentSong}`;
  ...
  test("#getBitRate - it should return the bitRate as string", async () => {
    ...
    jest
      .spyOn(service, service._executeClientStream.name)
    ...
    expect(service._executeClientStream).toHaveBeenCalledWith([
      "--i",
      "-B",
      song,
    ]);
  ...
  test("#_executeClientStream - it should call the sox command", async () => {
    const service = new Service();
    const spawnResponse = getSpawnResponse({
      stdout: "1k",
    });
    ...
    const args = ["myArgs"];
    const result = service._executeClientStream(args);
  ...
  test("#startStreamming - it should call the sox command", async () => {
    ...
    const expectedThrottle =
      config.constants.fallbackBitRate / config.constants.bitRateDivisor;
  ...
  test("#readFxByName - it should return the song", async () => {
    ...
    const expectedPath = `${config.dir.fxDirectory}/${fxOnDisk}`;
  ...
  test("#readFxByName - it should reject when song wasnt found", async () => {
    const service = new Service();
    const inputFx = "song01";
    jest.spyOn(fsPromises, fsPromises.readdir.name).mockResolvedValue([]);
